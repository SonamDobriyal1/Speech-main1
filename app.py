...
def _persist_sentence_to_firestore(sentence: dict[str, Any]) -> None:
    ...
    if client is None:
        return
    try:
        doc_ref = client.collection(PRACTICE_SENTENCES_COLLECTION).document(str(sentence["id"]))
        doc_ref.set({"id": sentence["id"], "text": sentence["text"]}, merge=True)
    except Exception as exc:  # pragma: no cover - remote call safety
        app.logger.warning("Failed to persist sentence: %s", exc)


def _delete_sentence_from_firestore(sentence_id: int) -> None:
    if firestore is None:
        return
    client = _get_firestore_client()
    if client is None:
        return
    try:
        client.collection(PRACTICE_SENTENCES_COLLECTION).document(str(sentence_id)).delete()
    except Exception as exc:  # pragma: no cover - remote call safety
        app.logger.warning("Failed to delete sentence: %s", exc)


def _persist_paragraph_to_firestore(paragraph: dict[str, Any]) -> None:
    if firestore is None:
        return
    client = _get_firestore_client()
    if client is None:
        return
    try:
        doc_ref = client.collection(PRACTICE_PARAGRAPHS_COLLECTION).document(str(paragraph["id"]))
        doc_ref.set({"id": paragraph["id"], "text": paragraph["text"]}, merge=True)
    except Exception as exc:  # pragma: no cover - remote call safety
        app.logger.warning("Failed to persist paragraph: %s", exc)


def _delete_paragraph_from_firestore(paragraph_id: int) -> None:
    if firestore is None:
        return
    client = _get_firestore_client()
    if client is None:
        return
    try:
        client.collection(PRACTICE_PARAGRAPHS_COLLECTION).document(str(paragraph_id)).delete()
    except Exception as exc:  # pragma: no cover - remote call safety
        app.logger.warning("Failed to delete paragraph: %s", exc)
...
def save_progress_entry(
    ...
    attention_average: Optional[float] = None,
) -> None:
    if not user_id:
        return
...
def transcribe():
    ...
    try:
        ...
        attention_raw = request.form.get("attentionAverage")
        if attention_raw:
            try:
                attention_value = float(attention_raw)
                if 0 <= attention_value <= 100:
                    attention_average = attention_value
            except ValueError:
                attention_average = None
...
def admin_users():
    ...
    if request.method == "POST":
        ...
        if action == "create":
            ...
            _admin_delete_sentence()
        elif action == "add_paragraph":
            _admin_add_paragraph()
        elif action == "delete_paragraph":
            _admin_delete_paragraph()
        else:
...
def _admin_add_sentence() -> None:
    text = (request.form.get("sentence_text") or "").strip()
    if not text:
        flash("Sentence text is required.", "error")
        return
    if len(text) > 300:
        flash("Sentence is too long (max 300 characters).", "error")
        return
    next_id = max((item["id"] for item in SENTENCES), default=0) + 1
    sentence = {"id": next_id, "text": text}
    SENTENCES.append(sentence)
    _persist_sentence_to_firestore(sentence)
    flash("Sentence added.", "success")


def _admin_delete_sentence() -> None:
    raw_id = (request.form.get("sentence_id") or "").strip()
    if not raw_id.isdigit():
        flash("Invalid sentence id.", "error")
        return
    sentence_id = int(raw_id)
    index = next((i for i, item in enumerate(SENTENCES) if item["id"] == sentence_id), None)
    if index is None:
        flash("Sentence not found.", "error")
        return
    SENTENCES.pop(index)
    _delete_sentence_from_firestore(sentence_id)
    flash("Sentence deleted.", "success")


def _admin_add_paragraph() -> None:
    text = (request.form.get("paragraph_text") or "").strip()
    if not text:
        flash("Paragraph text is required.", "error")
        return
    if len(text) > 1200:
        flash("Paragraph is too long (max 1200 characters).", "error")
        return
    next_id = max((item["id"] for item in PARAGRAPHS), default=100) + 1
    paragraph = {"id": next_id, "text": text}
    PARAGRAPHS.append(paragraph)
    _persist_paragraph_to_firestore(paragraph)
    flash("Paragraph added.", "success")


def _admin_delete_paragraph() -> None:
    raw_id = (request.form.get("paragraph_id") or "").strip()
    if not raw_id.isdigit():
        flash("Invalid paragraph id.", "error")
        return
    paragraph_id = int(raw_id)
    index = next((i for i, item in enumerate(PARAGRAPHS) if item["id"] == paragraph_id), None)
    if index is None:
        flash("Paragraph not found.", "error")
        return
    PARAGRAPHS.pop(index)
    _delete_paragraph_from_firestore(paragraph_id)
    flash("Paragraph deleted.", "success")
