(() => {
  ...
  const attentionAverageEl = document.getElementById('attentionAverage');
  ...
  function resetResults() {
    ...
    if (attentionAverageEl) {
      attentionAverageEl.textContent = '--%';
    }
  }
  ...
  async function startRecording() {
    ...
    if (window.AttentionTracker && typeof window.AttentionTracker.reset === 'function') {
      window.AttentionTracker.reset();
    }
  ...
  async function submitAudio(audioBlob) {
    ...
    let attentionAverage = null;
    if (window.AttentionTracker && typeof window.AttentionTracker.getAverage === 'function') {
      attentionAverage = window.AttentionTracker.getAverage();
      if (Number.isFinite(attentionAverage)) {
        formData.append('attentionAverage', String(attentionAverage));
      }
    }
    ...
    if (attentionAverageEl) {
      if (Number.isFinite(attentionAverage)) {
        attentionAverageEl.textContent = `${Math.round(attentionAverage)}%`;
      } else {
        attentionAverageEl.textContent = '--%';
      }
    }
